////  SourcesView.swift//  IPAOMTK////  Professional redesign for IPAOMTK//

import CoreDataimport AltSourceKitimport SwiftUIimport NimbleViews

struct SourcesView: View {#if !NIGHTLY && !DEBUG@AppStorage("Feather.shouldStar") private var _shouldStar: Int = 0#endif
...
private var _filteredSources: [AltSource] {
	_sources.filter {
		...
		($0.sourceURL?.absoluteString.localizedCaseInsensitiveContains(_searchText) ?? false)
	}
}
...
extension SourcesView {private var _headerView: some View {HStack(alignment: .center) {VStack(alignment: .leading, spacing: 6) {Text("Sources").font(.largeTitle.bold()).foregroundColor(.primary)
